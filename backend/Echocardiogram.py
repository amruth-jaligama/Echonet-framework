...
CONFIG_PATH = "C:\\Users\\Amruth Jaligama\\Downloads\\DS-CT\\backend"
...
class Segmentation(EchoCardiogram):
  ...
  def checkenvironment1(self):
    ...
    self.device = torch.device("cpu")
    self.checkpoint = torch.load(os.path.join(self.modeldir, os.path.basename(self.segmentationWeightsURL)), map_location = "cpu")
    self.state_dict_cpu = {k[7:]: v for (k, v) in self.checkpoint['state_dict'].items()}
    self.model.load_state_dict(self.state_dict_cpu)
  ...
  def outputvideo(self):
    fffvideo=cv.VideoCapture('./Videos/Output.avi')
    fffps = int(fffvideo.get(cv.CAP_PROP_FPS))
        ...
        with open(os.path.join(self.destination, "size.csv"), "w") as g:
            ...
            for (x, filename) in tqdm.tqdm(dataloader):
                ...
                for i in range(len(filename)):
                    ...
                    fig = plt.figure(figsize=(size.shape[0] / fffps * 1.5, 3))
                    plt.scatter(np.arange(size.shape[0]) / fffps, size, s=1)
                    ylim = plt.ylim()
                    for p in peaks:
                        plt.plot(np.array([p, p]) / fffps, ylim, linewidth=1)
                    plt.ylim(ylim)
                    ...
                    echonet.utils.savevideo(os.path.join(self.destination, "videos", filnm[0]), img.astype(np.uint8), fffps)
                    print("\n")
