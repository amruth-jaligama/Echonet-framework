...
def run(
    ...
    #nn.Linear () will help us initialize random values for weights and bias output
    model.fc = torch.nn.Linear(model.fc.in_features, 1) #No of output features is set to 1
    model.fc.bias.data[0] = 55.6
    if device.type == "cuda": # For fast computing 
        model = torch.nn.DataParallel(model)
    model.to(device)

    #bias unit guarantees activation of neuron(always 1)
    ...
    if weights is not None: 
        ...
        model.load_state_dict(checkpoint['state_dict']) # A state_dict is simply a Python dictionary object that maps each layer to its parameter tensor. 
    ...
    # Set up optimizer( Our optimizer here is SGD, which optimizes up the following model parameters and sets defaults)
